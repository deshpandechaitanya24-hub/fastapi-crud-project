...
def book_update(book: schemas.BookCreate, id: int, db: Session = Depends(get_db)):
...
@app.delete("/books/{id}", response_model=schemas.BookDelete)
def delete_book(id: int, db: Session = Depends(get_db)):
    delete_entry = services.delete_book(db, id)
    if delete_entry:
        return schemas.BookDelete(
            message=f"{delete_entry.title} deleted!"
        )
